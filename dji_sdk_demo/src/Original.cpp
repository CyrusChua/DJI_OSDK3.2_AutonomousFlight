...
static void Display_Main_Menu(void)
{
	...
	printf("| [3]  Release Control          | [19] Coordinate Test        |\n");
	printf("| [4]  Takeoff                  |                             |\n");
	printf("| [5]  Landing                  |                             |\n");
    printf("+-----------------------------------------------------------------+\n");
    printf("input 1/2/3/4/5/17/18/19 then press enter key\r\n");
    printf("----------------------------------------\r\n");
}
    ...
    while(1)
    {
        ...
        switch(main_operate_code)
        {
			...
			case 19:
			{
				...
				int a, i, o, R, L, M;
				double f, b, l, r, Front, Front2, Back, Left, Right, NFront, NFront2, NBack, NLeft, NRight, FR, FL, A, B, C, D, E;
				...
				virtual_rc_data[2] = 1024+75;	//2-> throttle 	[1024-660,1024+660]
				...
				for (i = 0; i < a; i++)
				{
					...
					if (x[i][0] > 0)
					{
					 E = x[i][0];
						...
						drone->virtual_rc_enable();
								usleep(20000);
								virtual_rc_data[0] = 1024;	//0-> roll     	[1024-660,1024+660]
								virtual_rc_data[1] = 1024+50;	//1-> pitch    	[1024-660,1024+660]
								...
								if ( NFront< 1.1)
								{
									ifstream FileFR("FrontRight.txt");
									ifstream FileFL("FrontLeft.txt");
									FileFR >> FR;
									FileFL >> FL;
									usleep(100000);
									E = E - C - B;
								...
								if ( FL > FR)
								{
										do
										{
											drone->virtual_rc_enable();
											usleep(20000);
											virtual_rc_data[0] = 1024-50;	//0-> roll     	[1024-660,1024+660]
											virtual_rc_data[1] = 1024;	//1-> pitch    	[1024-660,1024+660]
											virtual_rc_data[2] = 1024;	//2-> throttle 	[1024-660,1024+660]
											virtual_rc_data[3] = 1024;	//3-> yaw      	[1024-660,1024+660]
											virtual_rc_data[4] = 1684;	//4-> gear	{1684(UP), 1324(DOWN)}
					...
					else if (x[i][4] == 90)
					{
						...
						virtual_rc_data[1] = 1024;	//1-> pitch    	[1024-660,1024+660]
						virtual_rc_data[2] = 1024;	//2-> throttle 	[1024-660,1024+660]
						virtual_rc_data[3] = 1024-30;	//3-> yaw      	[1024-660,1024+660]
						virtual_rc_data[4] = 1684;	//4-> gear	{1684(UP), 1324(DOWN)}
						virtual_rc_data[6] = 1552;    	//6-> mode     	{1552(P), 1024(A), 496(F)}
					for (int p = 0; p < 3010; p++)
						{
